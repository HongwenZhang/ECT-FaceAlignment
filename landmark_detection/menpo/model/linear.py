...
class LinearVectorModel(Copyable):
    ...
    def project(self, vector, project_weight=None):
        """
        ...
        """
        return self.project_vectors(vector[None, :], project_weight).flatten()
...
class MeanLinearVectorModel(LinearVectorModel):
    ...
    def project_vectors(self, vectors, project_weight=None):
        """
        Projects each of the `vectors` onto the model, retrieving
        the optimal linear reconstruction weights for each instance.
        ...
        if project_weight is None:
            ...
            # pass print 'Used projection w/o weight.'
        else:
            if vectors.shape == project_weight.shape:
                X *= project_weight
                print 'Used projection with weight.'
                return np.dot(X, np.linalg.inv(self.components.dot(np.diag(project_weight[0])).dot(self.components.T)).dot(self.components).T)

            else:
                raise ValueError(
                    "project_weight's shape {} doesn't match "
                    "vectors' shape {}.".format(project_weight.shape, vectors.shape))
        # return np.dot(X, self.components.T)
