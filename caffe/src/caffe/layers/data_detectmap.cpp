...
#include "caffe/layers/data_detectmap.hpp"
...
DataDetectmapLayer<Dtype>::~DataDetectmapLayer<Dtype>() {
    this->StopInternalThread();
}


template<typename Dtype>
void DataDetectmapLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top) {
    DetectmapDataParameter detectmap_data_param = this->layer_param_.detectmap_data_param();

    // Shortcuts
    const std::string labelindsStr = detectmap_data_param.labelinds();
    const int batchsize = detectmap_data_param.batchsize();
    const int label_width = detectmap_data_param.label_width();
    const int label_height = detectmap_data_param.label_height();
    const int size = detectmap_data_param.cropsize();
    const int outsize = detectmap_data_param.outsize();
    const int label_batchsize = batchsize;
    sample_per_cluster_ = detectmap_data_param.sample_per_cluster();
    root_img_dir_ = detectmap_data_param.root_img_dir();
    ...
    // load GT
    std::string gt_path = detectmap_data_param.source();
    ...
    if (!detectmap_data_param.has_meanfile())
    {
        ...
        sub_mean_ = true;
        string mean_path = detectmap_data_param.meanfile();
...
template<typename Dtype>
void DataDetectmapLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
    ...
    const Dtype scale = detectmap_data_param.scale();
    const int batchsize = detectmap_data_param.batchsize();
    const int label_height = detectmap_data_param.label_height();
    const int label_width = detectmap_data_param.label_width();
    const float angle_max = detectmap_data_param.angle_max();
    const bool dont_flip_first = detectmap_data_param.dont_flip_first();
    const bool flip_joint_labels = detectmap_data_param.flip_joint_labels();
    const int multfact = detectmap_data_param.multfact();
    const bool segmentation = detectmap_data_param.segmentation();
    const int size = detectmap_data_param.cropsize();
    const int outsize = detectmap_data_param.outsize();
    const int num_aug = 1;
    const float resizeFact = (float)outsize / (float)size;
    const bool random_crop = detectmap_data_param.random_crop();
	const int detect_radius = detectmap_data_param.detect_radius();
    ...
    for (int idx_img = 0; idx_img < batchsize; idx_img++)
    {
        ...
        for (int idx_aug = 0; idx_aug < num_aug; idx_aug++)
        {
            ...
            float sigma = detect_radius;    //1.5 modify to 3   zhw 160817
            ...
            for (int idx_ch = 0; idx_ch < label_num_channels / 2; idx_ch++)
            {
                ...
                for (int i = 0; i < label_height; i++)
                {
                    for (int j = 0; j < label_width; j++)
                    {
						...
						float gaussian = 0;
						if(( pow(i - y, 2.0) + pow(j - x, 2.0) ) < pow(sigma, 2.0) ){
							gaussian = 1;
						}
                        //float gaussian = ( 1 / ( sigma * sqrt(2 * M_PI) ) ) * exp( -0.5 * ( pow(i - y, 2.0) + pow(j - x, 2.0) ) * pow(1 / sigma, 2.0) );
                        //gaussian = 2.667 * sigma * gaussian;       //4 to 8 corresposed to sigma=3
...
template<typename Dtype>
void DataDetectmapLayer<Dtype>::GetCurImg(string& img_name, std::vector<float>& img_label, std::vector<float>& crop_info, int& img_class)
{
...
template<typename Dtype>
void DataDetectmapLayer<Dtype>::AdvanceCurImg()
{
    if (!sample_per_cluster_)
    {
...
template<typename Dtype>
void DataDetectmapLayer<Dtype>::VisualiseAnnotations(cv::Mat img_annotation_vis, int label_num_channels, std::vector<float>& img_class, int multfact)
{
    // colors
...
template <typename Dtype>
float DataDetectmapLayer<Dtype>::Uniform(const float min, const float max) {
...
template <typename Dtype>
cv::Mat DataDetectmapLayer<Dtype>::RotateImage(cv::Mat src, float rotation_angle)
{
...
INSTANTIATE_CLASS(DataDetectmapLayer);
REGISTER_LAYER_CLASS(DataDetectmap);
