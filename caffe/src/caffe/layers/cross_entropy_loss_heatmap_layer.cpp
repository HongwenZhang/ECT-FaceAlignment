...
#include "caffe/layers/cross_entropy_loss_heatmap_layer.hpp"
...
void CrossEntropyLossHeatmapLayer<Dtype>::Reshape(
    ...
    p_div_.Reshape(bottom[0]->num(), bottom[0]->channels(),
                  bottom[0]->height(), bottom[0]->width());
    ones_.Reshape(bottom[0]->num(), bottom[0]->channels(),
                  bottom[0]->height(), bottom[0]->width());
    oneSubp_gt_.Reshape(bottom[0]->num(), bottom[0]->channels(),
                  bottom[0]->height(), bottom[0]->width());
    oneSubp_pred_.Reshape(bottom[0]->num(), bottom[0]->channels(),
                  bottom[0]->height(), bottom[0]->width());
    oneSubp_div_.Reshape(bottom[0]->num(), bottom[0]->channels(),
                  bottom[0]->height(), bottom[0]->width());
    logp_pred_.Reshape(bottom[0]->num(), bottom[0]->channels(),
                  bottom[0]->height(), bottom[0]->width());
    logoneSubp_pred_.Reshape(bottom[0]->num(), bottom[0]->channels(),
                  bottom[0]->height(), bottom[0]->width());
...
template<typename Dtype>
void CrossEntropyLossHeatmapLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void CrossEntropyLossHeatmapLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
        ...
        for (int idx_ch = 0; idx_ch < num_channels; idx_ch++)
        {
            for (int i = 0; i < label_height; i++)
            {
                for (int j = 0; j < label_width; j++)
                {
                    ...
                    // loss += diff * diff;
                    float p_gt = (float)gt_pred[image_idx];
                    float p_pred = (float)bottom_pred[image_idx];
                    float diff = p_gt*log(std::max(bottom_pred[image_idx],Dtype(FLT_MIN))) + (1-p_gt)*log(std::max((1-bottom_pred[image_idx]),Dtype(FLT_MIN)));
                    loss -= diff;
...
template <typename Dtype>
void CrossEntropyLossHeatmapLayer<Dtype>::Visualise(float loss, cv::Mat bottom_img, cv::Mat gt_img, cv::Mat diff_img, std::vector<cv::Point>& points, cv::Size size)
{
    DLOG(INFO) << loss;
...
template <typename Dtype>
void CrossEntropyLossHeatmapLayer<Dtype>::VisualiseBottom(const vector<Blob<Dtype>*>& bottom, int idx_img, int visualise_channel, std::vector<cv::Point>& points, cv::Size size)
{
...
template <typename Dtype>
void CrossEntropyLossHeatmapLayer<Dtype>::PrepVis(cv::Mat img, cv::Size size)
{
...
template <typename Dtype>
void CrossEntropyLossHeatmapLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
        const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom)
{
    ...
    Dtype* p_div = p_div_.mutable_cpu_data();
    Dtype* ones = ones_.mutable_cpu_data();
    Dtype* oneSubp_gt = oneSubp_gt_.mutable_cpu_data();
    Dtype* oneSubp_pred = oneSubp_pred_.mutable_cpu_data();
    Dtype* oneSubp_div = oneSubp_div_.mutable_cpu_data();
    caffe_div(count, bottom[1]->cpu_data(), bottom[0]->cpu_data(), p_div);
    caffe_powx(count, bottom[0]->cpu_data(), Dtype(0), ones);
    caffe_sub(count, ones, bottom[1]->cpu_data(), oneSubp_gt);
    caffe_sub(count, ones, bottom[0]->cpu_data(), oneSubp_pred);
    caffe_div(count, oneSubp_gt, oneSubp_pred, oneSubp_div);

    caffe_sub(count, oneSubp_div, p_div, bottom[0]->mutable_cpu_diff());

    Dtype* logp_pred = logp_pred_.mutable_cpu_data();
    Dtype* logoneSubp_pred = logoneSubp_pred_.mutable_cpu_data();
    caffe_log(count, bottom[0]->cpu_data(), logp_pred);
    caffe_log(count, oneSubp_pred, logoneSubp_pred);

    caffe_sub(count, logp_pred, logoneSubp_pred, bottom[1]->mutable_cpu_diff());
...
#ifndef CPU_ONLY
template <typename Dtype>
void CrossEntropyLossHeatmapLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void CrossEntropyLossHeatmapLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
        const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom)
{
...
#ifdef CPU_ONLY
STUB_GPU(CrossEntropyLossHeatmapLayer);
#endif

INSTANTIATE_CLASS(CrossEntropyLossHeatmapLayer);
REGISTER_LAYER_CLASS(CrossEntropyLossHeatmap);
