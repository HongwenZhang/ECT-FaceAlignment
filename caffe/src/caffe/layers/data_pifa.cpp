...
#include "caffe/layers/data_pifa.hpp"
...
DataPifaLayer<Dtype>::~DataPifaLayer<Dtype>() {
    this->StopInternalThread();
}


template<typename Dtype>
void DataPifaLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top) {
	PifaDataParameter pifa_data_param = this->layer_param_.pifa_data_param();

    // Shortcuts
    const std::string labelindsStr = pifa_data_param.labelinds();
    const int batchsize = pifa_data_param.batchsize();
    const int label_width = pifa_data_param.label_width();
    const int label_height = pifa_data_param.label_height();
    const int size = pifa_data_param.cropsize();
    const int outsize = pifa_data_param.outsize();
    const int label_batchsize = batchsize;
    sample_per_cluster_ = pifa_data_param.sample_per_cluster();
    root_img_dir_ = pifa_data_param.root_img_dir();
    ...
    // load GT
    std::string gt_path = pifa_data_param.source();
    LOG(INFO) << "Loading annotation from " << gt_path;

    std::ifstream infile(gt_path.c_str());
    string img_name, labels, visLabels;
    //if (!sample_per_cluster_)
    //{
        // sequential sampling
        while (infile >> img_name >> labels >> visLabels)
        {
            ...
            // read cropping info
            std::vector <float> visInfo;
            std::istringstream ss2(visLabels);
            while (ss2)
            {
                std::string s;
                if (!std::getline(ss2, s, ',')) break;
                visInfo.push_back(atof(s.c_str()));
            }

            //int clusterClass = atoi(clusterClassStr.c_str());
            img_label_list_.push_back(std::make_pair(img_name, std::make_pair(label, visInfo)));
        }
    ...
    /* else
    {
        ...
        while (infile >> img_name >> labels >> visLabels )
        {
            ...
            img_list_[clusterClass].push_back(std::make_pair(img_name, std::make_pair(label, std::make_pair(visInfo, clusterClass))));
        }
    ...
    if (!pifa_data_param.has_meanfile())
    {
        ...
        sub_mean_ = true;
        string mean_path = pifa_data_param.meanfile();
...
template<typename Dtype>
void DataPifaLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
    ...
    const Dtype scale = pifa_data_param.scale();
    const int batchsize = pifa_data_param.batchsize();
    const int label_height = pifa_data_param.label_height();
    const int label_width = pifa_data_param.label_width();
    const float angle_max = pifa_data_param.angle_max();
    const bool dont_flip_first = pifa_data_param.dont_flip_first();
    const bool flip_joint_labels = pifa_data_param.flip_joint_labels();
    const int multfact = pifa_data_param.multfact();
    const bool segmentation = pifa_data_param.segmentation();
    const int size = pifa_data_param.cropsize();
    const int outsize = pifa_data_param.outsize();
    const int num_aug = 1;
    const float resizeFact = (float)outsize / (float)size;
    const bool random_crop = pifa_data_param.random_crop();
...
//    else flip_start_ind = 0;
    ...
    for (int idx_img = 0; idx_img < batchsize; idx_img++)
    {
        ...
        this->GetCurImg(img_name, cur_label, cur_cropinfo);
		...
		//LOG(INFO) << "imgtype: " << img.type()<<"::"<<CV_8UC1;
        ...
        for (int idx_aug = 0; idx_aug < num_aug; idx_aug++)
        {
            ...
            if (random_crop)
            {
                ...
                if (rand() % 2)
                {
                    ...
                    if (flip_joint_labels)
                    {
						...
						//flip check   flip 29 point  COFWs
						for (int i = 0; i < 15; i ++)
                        {
                            int ind = 2*i;
                            int ind_offset = 0;
                            if (ind == 20||ind == 24||ind == 26||ind == 28||ind == 6||ind == 14){
                                continue;
                            }
                            if (ind == 4||ind == 12){
                                ind_offset = 2;
                                cur_label_aug[2*ind] = cur_label_copy[2*(ind+ind_offset)];
                                cur_label_aug[2*ind+1] = cur_label_copy[2*(ind+ind_offset)+1];
                                cur_label_aug[2*(ind+ind_offset)] = cur_label_copy[2*ind];
                                cur_label_aug[2*(ind+ind_offset)+1] = cur_label_copy[2*ind+1];
                                ind += 1;
                                cur_label_aug[2*ind] = cur_label_copy[2*(ind+ind_offset)];
                                cur_label_aug[2*ind+1] = cur_label_copy[2*(ind+ind_offset)+1];
                                cur_label_aug[2*(ind+ind_offset)] = cur_label_copy[2*ind];
			...
			if (label_num_channels/2 != cur_cropinfo.size()){
				LOG(ERROR) << "number of visible label doesn't match.";
			}
			else {
				for (int idx_ch = 0; idx_ch < label_num_channels / 2; idx_ch++)
				{
					...
					for (int i = 0; i < label_height; i++)
					{
						for (int j = 0; j < label_width; j++)
						{
							...
							if(cur_cropinfo[idx_ch]==1)
								gaussian = 0;

							top_label[label_idx] = gaussian;


							if (dataMatrix.at<float>((int)i, (int)j) < gaussian)
				...
				if (visualise)
	            {
					cv::imshow("label image", dataMatrix);
				}
			}
...
template<typename Dtype>
void DataPifaLayer<Dtype>::GetCurImg(string& img_name, std::vector<float>& img_label, std::vector<float>& crop_info)
{
...
template<typename Dtype>
void DataPifaLayer<Dtype>::AdvanceCurImg()
{
...
template<typename Dtype>
void DataPifaLayer<Dtype>::VisualiseAnnotations(cv::Mat img_annotation_vis, int label_num_channels, std::vector<float>& img_class, int multfact)
{
    // colors
...
template <typename Dtype>
float DataPifaLayer<Dtype>::Uniform(const float min, const float max) {
...
template <typename Dtype>
cv::Mat DataPifaLayer<Dtype>::RotateImage(cv::Mat src, float rotation_angle)
{
...
INSTANTIATE_CLASS(DataPifaLayer);
REGISTER_LAYER_CLASS(DataPifa);
